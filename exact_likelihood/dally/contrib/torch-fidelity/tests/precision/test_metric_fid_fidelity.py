...
class TestMetricFidFidelity(unittest.TestCase):
    ...
    def test_fid_pt_tf_fidelity(self):
        ...
        limit = 5000
        cifar10train_root = os.path.join(
            tempfile.gettempdir(), f"cifar10-train-img-{limit}"
        )
        cifar10valid_root = os.path.join(
            tempfile.gettempdir(), f"cifar10-valid-img-{limit}"
        )
        ...
        res_ref = self.call_ref_fid(
            cifar10train_root, cifar10valid_root, cuda, determinism=True
        )
