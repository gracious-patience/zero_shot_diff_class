...
class TestMetricKidFidelity(unittest.TestCase):
    ...
    def call_ref_kid(input_1, input_2, cuda, determinism):
        args = [
            "python3",
            "reference/reference_metric_kid_mmdgan.py",
            input_1,
            input_2,
            "--no-inception",
            "--do-mmd",
    ...
    def test_kid_pt_tf_fidelity(self):
        ...
        limit = 5000
        cifar10trainorig_root = os.path.join(
            tempfile.gettempdir(), f"cifar10-train-img-{limit}"
        )
        ...
        res = subprocess.run(
            (
                "python3",
                "utils/util_dump_dataset_as_images.py",
                "cifar10-train",
        ...
        res_ref = self.call_ref_kid(
            cifar10validnoise_root, cifar10trainorig_codes, cuda, determinism=True
        )
        ...
        res_fidelity = self.call_fidelity_kid(
            cifar10validnoise_root, cifar10trainorig_root
        )
        ...
        err_abs_mean = abs(
            res_ref[KEY_METRIC_KID_MEAN] - res_fidelity[KEY_METRIC_KID_MEAN]
        )
        err_abs_std = abs(
            res_ref[KEY_METRIC_KID_STD] - res_fidelity[KEY_METRIC_KID_STD]
        )
