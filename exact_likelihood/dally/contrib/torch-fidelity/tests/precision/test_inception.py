...
class TestInception(unittest.TestCase):
    ...
    def get_inception_tf():
        ...
        if not os.path.exists(filepath):
            ...
            print(
                f"Succesfully downloaded {filename} {statinfo.st_size} bytes.",
                file=sys.stderr,
            )
    ...
    def forward_pt(model, x_pt_bchw, cuda):
        ...
        return (
            featuresdict["2048"].cpu().numpy(),
            featuresdict["logits_unbiased"].cpu().numpy(),
        )
    ...
    def test_inception_tolerances_and_determinism(self):
        ...
        def process_feature_pair(featvec_pt, featvec_tf):
            f_pt = np.concatenate(
                [np.expand_dims(a, 0) for a in featvec_pt], 0
            )  # repeats x batch x feat_dims ..
            f_tf = np.concatenate([np.expand_dims(a, 0) for a in featvec_tf], 0)
            return {
                "max_pixelwise_difference_across_runs_pt": np.max(
                    np.max(f_pt, axis=0) - np.min(f_pt, axis=0)
                ),
                "max_pixelwise_difference_across_runs_tf": np.max(
                    np.max(f_tf, axis=0) - np.min(f_tf, axis=0)
                ),
                "max_pixelwise_err_abs_pt_tf": np.max(np.abs(f_pt - f_tf)),
                "max_pixelwise_err_rel_pt_tf": np.max(
                    np.abs(f_pt - f_tf) / max(np.max(np.abs(f_tf)), 1e-9)
                ),
            }
        ...
        if cuda:
            ...
            for fname, fstat in fstats.items():
                ...
                for sname, sval in fstat.items():
                    ...
                    print(f"{name}: {sval}", file=sys.stderr)
                    if sname in (
                        "max_pixelwise_difference_across_runs_pt",
                        "max_pixelwise_difference_across_runs_tf",
                    ):
                        self.assertEqual(sval, 0, name)
                    elif sname == "max_pixelwise_err_abs_pt_tf":
                        self.assertLessEqual(sval, 1e-3, name)
