...
class TestInterpolation(unittest.TestCase):
    ...
    def resize_pt(img, size, cuda, cast_back_uint=True, method=None):
        ...
        if cuda:
            img_in = img_in.cuda()
        img_out = interpolate_bilinear_2d_like_tensorflow1x(
            img_in, size, align_corners=False, method=method
        )
    ...
    def _max_resize_residual(self, cuda, method, visualize=None, suffix=""):
        ...
        img_299_pt = self.resize_pt(
            img_032, (299, 299), cuda, cast_back_uint=False, method=method
        )
