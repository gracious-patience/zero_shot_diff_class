...
class TestConvolution(unittest.TestCase):
    ...
    def forward_tf(conv_pt, x_pt):
        ...
        weight_tf = (
            conv_pt.weight.permute(2, 3, 1, 0).cpu().numpy()
    ...
    def estimate_implementation_exactness(self, cuda):
        ...
        if cuda:
            x_pt = x_pt.cuda()
        x_pt = x_pt.float()
        x_pt = interpolate_bilinear_2d_like_tensorflow1x(
            x_pt, size=(299, 299), align_corners=False
        )
        x_pt = (x_pt - 128) / 128
        ...
        self.save(
            err_abs_pt_builtin_manualchw, f"{suffix}_err_abs_pt_builtin_manualchw.png"
        )
        self.save(
            err_abs_pt_builtin_manualhwc, f"{suffix}_err_abs_pt_builtin_manualhwc.png"
        )
