...
def featurize(
    ...
    if isinstance(model, Inception):
        ...
        if verbose and (hi > 255 or lo < 0):
            print(
                f"WARNING! Inception min/max violated: min={lo}, max={hi}",
                file=sys.stderr,
            )
        ...
        if verbose and (hi > 0.5 or lo < -0.5):
            print(
                f"WARNING! LeNet min/max violated: min={lo}, max={hi}", file=sys.stderr
            )
...
def main():
    ...
    parser.add_argument(
        "--reference-subset",
        default=slice(None),
        type=lambda x: slice(*(int(s) if s else None for s in x.split(":"))),
    )
    ...
    parser.add_argument(
        "--split-method", choices=["openai", "bootstrap"], default="openai"
    )
    ...
    if os.path.isdir(args.samples):
        files = glob_images_path(
            args.samples, glob_recursively=False, verbose=not args.silent
        )
        samples = [
            np.expand_dims(np.array(Image.open(f).convert("RGB")), 0) for f in files
        ]
    ...
    if not args.silent:
        print(
            "Transformer test: transformer([-1, 0, 10.]) = "
            + repr(transformer(np.array([-1, 0, 10.0]))),
            file=sys.stderr,
        )
