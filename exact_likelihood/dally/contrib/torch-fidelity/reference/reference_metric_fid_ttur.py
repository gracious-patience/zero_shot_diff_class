...
def calculate_activation_statistics_from_files(
    ...
    -- sigma : The covariance matrix of the activations of the pool_3 layer of
               the incption model.
    """
    act = get_activations_from_files(
        files, sess, batch_size=batch_size, verbose=verbose
    )
    mu = np.mean(act, axis=0)
    sigma = np.cov(act, rowvar=False)
    return mu, sigma
...
def handle_path(path, sess, low_profile=False, verbose=False):
    if path.endswith(".npz"):
        ...
        if low_profile:
            m, s = calculate_activation_statistics_from_files(
                files, sess, verbose=verbose
            )
        else:
...
if __name__ == "__main__":
    ...
    metrics = calculate_fid_given_paths(
        args.path, low_profile=args.lowprofile, verbose=not args.silent
    )
