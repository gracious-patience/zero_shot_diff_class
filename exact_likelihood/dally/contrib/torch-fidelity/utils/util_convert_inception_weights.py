...
def check_or_download_inception_graphdef():
    ...
    model_file = pathlib.Path(
        os.path.join(tempfile.gettempdir(), "classify_image_graph_def.pb")
    )
...
def convert_tensorflow_graphdef_to_pytorch_checkpoint(path_out=None):
    ...
    for k_tf, k_pt in map_tf_to_pt.items():
        if k_tf.endswith("conv2d_params"):
            ...
            weights_pt[k_pt + ".weight"] = torch.from_numpy(
                weights_tf.pop(k_tf + "/gamma")
            )
            weights_pt[k_pt + ".bias"] = torch.from_numpy(
                weights_tf.pop(k_tf + "/beta")
            )
            weights_pt[k_pt + ".running_mean"] = torch.from_numpy(
                weights_tf.pop(k_tf + "/moving_mean")
            )
            weights_pt[k_pt + ".running_var"] = torch.from_numpy(
                weights_tf.pop(k_tf + "/moving_variance")
            )
            ...
            raise NotImplementedError(
                f"Cannot handle TensorFlow GraphDef Const item {k_tf}"
            )
...
if __name__ == "__main__":
    print(
        "Converting TensorFlow InceptionV3 pretrained weights to a PyTorch checkpoint..."
    )
