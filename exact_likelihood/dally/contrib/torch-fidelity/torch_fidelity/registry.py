...
def register_feature_extractor(name, cls):
    ...
    vassert(
        name not in FEATURE_EXTRACTORS_REGISTRY,
...
def register_sample_similarity(name, cls):
    ...
    vassert(
        name not in SAMPLE_SIMILARITY_REGISTRY,
        f'Sample similarity "{name}" is already registered',
    )
    vassert(
        issubclass(cls, SampleSimilarityBase),
        "Sample similarity class must be subclassed from SampleSimilarityBase",
    )
...
def register_noise_source(name, fn_generate):
    ...
    vassert(
        name not in NOISE_SOURCE_REGISTRY,
        f'Noise source "{name}" is already registered',
    )
    vassert(
...
def register_interpolation(name, fn_interpolate):
    ...
    vassert(
        name not in INTERPOLATION_REGISTRY,
        f'Interpolation "{name}" is already registered',
    )
    vassert(
...
register_dataset(
    "cifar10-train",
    lambda root, download: Cifar10_RGB(
        root, train=True, transform=TransformPILtoRGBTensor(), download=download
    ),
)
register_dataset(
    "cifar10-val",
    lambda root, download: Cifar10_RGB(
        root, train=False, transform=TransformPILtoRGBTensor(), download=download
    ),
)
register_dataset(
    "stl10-train",
    lambda root, download: STL10_RGB(
        root, split="train", transform=TransformPILtoRGBTensor(), download=download
    ),
)
register_dataset(
    "stl10-test",
    lambda root, download: STL10_RGB(
        root, split="test", transform=TransformPILtoRGBTensor(), download=download
    ),
)
register_dataset(
    "stl10-unlabeled",
    lambda root, download: STL10_RGB(
        root, split="unlabeled", transform=TransformPILtoRGBTensor(), download=download
    ),
)
