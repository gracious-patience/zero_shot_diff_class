...
def glob_samples_paths(
    path, samples_find_deep, samples_find_ext, samples_ext_lossy=None, verbose=True
):
    vassert(
        type(samples_find_ext) is str and samples_find_ext != "",
        "Sample extensions not specified",
    )
    vassert(
        samples_ext_lossy is None or type(samples_ext_lossy) is str,
        "Lossy sample extensions can be None or string",
    )
    vprint(
        verbose,
    ...
    samples_find_ext = [
        a.strip() for a in samples_find_ext.split(",") if a.strip() != ""
    ]
    if samples_ext_lossy is not None:
        samples_ext_lossy = [
            a.strip() for a in samples_ext_lossy.split(",") if a.strip() != ""
        ]
    have_lossy = False
    files = []
...
def create_feature_extractor(name, list_features, cuda=True, **kwargs):
    vassert(
        name in FEATURE_EXTRACTORS_REGISTRY,
        f'Feature extractor "{name}" not registered',
    )
    vprint(
        get_kwarg("verbose", kwargs),
        f'Creating feature extractor "{name}" with features {list_features}',
    )
...
def create_sample_similarity(name, cuda=True, **kwargs):
    vassert(
        name in SAMPLE_SIMILARITY_REGISTRY, f'Sample similarity "{name}" not registered'
    )
...
def get_featuresdict_from_dataset(
    input, feat_extractor, batch_size, cuda, save_cpu_ram, verbose
):
    ...
    vassert(
        isinstance(feat_extractor, FeatureExtractorBase),
...
def get_featuresdict_from_generative_model(
    gen_model, feat_extractor, num_samples, batch_size, cuda, rng_seed, verbose
):
    vassert(
        isinstance(gen_model, GenerativeModelBase),
        "Input can only be a GenerativeModel instance",
    )
    vassert(
        isinstance(feat_extractor, FeatureExtractorBase),
        "Feature extractor is not a subclass of FeatureExtractorBase",
    ...
    with tqdm(
        ...
        for sample_start in range(0, num_samples, batch_size):
            ...
            if gen_model.num_classes > 0:
                cond_labels = torch.from_numpy(
                    rng.randint(
                        low=0, high=gen_model.num_classes, size=(sz,), dtype=np.int
                    )
                )
                if cuda:
        ...
        if input in DATASETS_REGISTRY:
            ...
            if datasets_root is None:
                datasets_root = os.path.join(
                    torch.hub._get_torch_home(), "fidelity_datasets"
                )
            ...
            input = glob_samples_paths(
                input, samples_find_deep, samples_find_ext, samples_ext_lossy, verbose
            )
            vassert(
                len(input) > 0,
                f"No samples found in {input} with samples_find_deep={samples_find_deep}",
            )
...
def prepare_input_descriptor_from_input_id(input_id, **kwargs):
    vassert(
        ...
        or type(input_id) is str
        and input_id in DATASETS_REGISTRY,
...
def cache_lookup_group_recompute_all_on_any_miss(
    cached_filename_prefix, item_names, fn_recompute, **kwargs
):
    verbose = get_kwarg("verbose", kwargs)
    if not get_kwarg("cache", kwargs):
        return fn_recompute()
    ...
    os.makedirs(cache_root, exist_ok=True)
    cached_paths = [
        os.path.join(cache_root, cached_filename_prefix + a + ".pt") for a in item_names
    ]
...
def extract_featuresdict_from_input_id(input_id, feat_extractor, **kwargs):
    ...
    if isinstance(input, Dataset):
        ...
        vassert(
            type(num_samples) is int and num_samples > 0,
...
def extract_featuresdict_from_input_id_cached(input_id, feat_extractor, **kwargs):
    ...
    if cacheable_input_name is not None:
        ...
        cached_filename_prefix = (
            f"{cacheable_input_name}-{feat_extractor_name}-features-"
        )
