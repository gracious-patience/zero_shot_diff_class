...
def fid_statistics_to_metric(stat_1, stat_2, verbose):
    ...
    if not np.isfinite(covmean).all():
        ...
        covmean = scipy.linalg.sqrtm(
            (sigma1 + offset).dot(sigma2 + offset), disp=verbose
        )
...
def fid_featuresdict_to_statistics_cached(
    ...
    if cacheable_input_name is not None:
        ...
        cached_name = (
            f"{cacheable_input_name}-{feat_extractor_name}-stat-fid-{feat_layer_name}"
        )
...
def fid_input_id_to_statistics_cached(
    input_id, feat_extractor, feat_layer_name, **kwargs
):
    def fn_recompute():
        return fid_input_id_to_statistics(
            input_id, feat_extractor, feat_layer_name, **kwargs
        )
...
def fid_inputs_to_metric(feat_extractor, **kwargs):
    ...
    stats_1 = fid_input_id_to_statistics_cached(
        1, feat_extractor, feat_layer_name, **kwargs
    )

    vprint(verbose, f"Extracting statistics from input 2")
    stats_2 = fid_input_id_to_statistics_cached(
        2, feat_extractor, feat_layer_name, **kwargs
    )
...
def calculate_fid(**kwargs):
    ...
    feat_extractor = create_feature_extractor(
        feature_extractor, [feat_layer_name], **kwargs
    )
