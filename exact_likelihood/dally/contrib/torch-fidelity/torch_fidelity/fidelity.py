...
def main():
    ...
    pgroup.add_argument(
        "-c", "--cpu", action="store_true", help="Use CPU despite capabilities"
    )
    parser.add_argument(
        "-j", "--json", action="store_true", help="Print scores in JSON"
    )
    parser.add_argument(
        "-i", "--isc", action="store_true", help="Calculate ISC (Inception Score)"
    )
    parser.add_argument(
        "-f",
        "--fid",
    ...
    parser.add_argument(
        "--isc-splits",
        default=DEFAULTS["isc_splits"],
        type=int,
        help="Number of splits in ISC",
    )
    parser.add_argument(
        "--kid-subsets",
        default=DEFAULTS["kid_subsets"],
        type=int,
        help="Number of subsets in KID",
    )
    parser.add_argument(
        "--kid-subset-size",
        default=DEFAULTS["kid_subset_size"],
        type=int,
        help="Subset size in KID",
    )
    parser.add_argument(
        "--kid-degree",
        default=DEFAULTS["kid_degree"],
        type=int,
        help="Degree of polynomial kernel in KID",
    )
    parser.add_argument(
        "--kid-gamma",
        default=DEFAULTS["kid_gamma"],
        type=float,
        help="Polynomial kernel gamma in KID",
    )
    parser.add_argument(
        "--kid-coef0",
        default=DEFAULTS["kid_coef0"],
        type=float,
        help="Polynomial kernel coef0 in KID",
    )
    parser.add_argument(
        "--ppl-epsilon",
        default=DEFAULTS["ppl_epsilon"],
        type=float,
    ...
    if type(unknown) is list and len(unknown) > 0:
        ...
        print(
            f"  This may be due the command line options change in the most recent version,",
            file=sys.stderr,
        )
        print(
            f"  Use " "fidelity --help" " to see the up-to-date command line options,",
            file=sys.stderr,
        )
        print(
            f"  See https://github.com/toshas/torch-fidelity/blob/master/CHANGELOG.md",
            file=sys.stderr,
        )
