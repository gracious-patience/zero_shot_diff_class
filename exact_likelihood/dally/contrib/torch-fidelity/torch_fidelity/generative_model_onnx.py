...
class GenerativeModelONNX(GenerativeModelBase):
    def __init__(self, path_onnx, z_size, z_type, num_classes):
        ...
        vassert(
            z_type in ("normal", "unit", "uniform_0_1"),
    ...
    def forward(self, *args):
        ...
        vassert(
            all(torch.is_tensor(a) for a in args), "All model inputs must be tensors"
        )
        ort_input = {
            self.input_names[i]: self.to_numpy(args[i]) for i in range(len(args))
        }
