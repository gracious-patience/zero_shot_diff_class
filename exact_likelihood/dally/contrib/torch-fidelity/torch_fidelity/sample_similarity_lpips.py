    ...
    def __init__(self):
        ...
        with redirect_stdout(sys.stderr):
            vgg_pretrained_features.load_state_dict(
                load_state_dict_from_url(URL_VGG16_BASE)
            )
...
class SampleSimilarityLPIPS(SampleSimilarityBase):
        ...
        **kwargs,
        ...
        with redirect_stdout(sys.stderr):
            state_dict = load_state_dict_from_url(
                URL_VGG16_LPIPS, map_location="cpu", progress=True
            )
    ...
    def normalize(x):
        ...
        mean_rescaled = (
            (
                1
                + torch.tensor([-0.030, -0.088, -0.188], device=x.device)[
                    None, :, None, None
                ]
            )
            * 255
        ...
        inv_std_rescaled = 2 / (
            torch.tensor([0.458, 0.448, 0.450], device=x.device)[None, :, None, None]
            * 255
        )
    ...
    def forward(self, in0, in1):
        vassert(
            torch.is_tensor(in0) and torch.is_tensor(in1),
        ...
        if self.sample_similarity_dtype is not None:
            ...
            vassert(
                dtype is not None and in0.dtype == dtype and in1.dtype == dtype,
