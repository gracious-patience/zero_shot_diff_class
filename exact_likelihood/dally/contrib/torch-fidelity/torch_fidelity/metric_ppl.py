...
def calculate_ppl(input_id, **kwargs):
    ...
    vassert(
        discard_percentile_higher is None or 0 < discard_percentile_higher < 100,
        "Invalid percentile",
    )
    if discard_percentile_lower is not None and discard_percentile_higher is not None:
        vassert(
            0 < discard_percentile_lower < discard_percentile_higher < 100,
            "Invalid percentiles",
        )
    ...
    lat_e0 = sample_random(
        rng, (input_model_num_samples, input_model_z_size), input_model_z_type
    )
    lat_e1 = sample_random(
        rng, (input_model_num_samples, input_model_z_size), input_model_z_type
    )
    ...
    if is_cond:
        labels = torch.from_numpy(
            rng.randint(0, input_model_num_classes, (input_model_num_samples,))
        )
