...
def train(args):
    ...
    scheduler_G = torch.optim.lr_scheduler.LambdaLR(
        optim_G, lambda step: 1.0 - step / args.num_total_steps
    )
    scheduler_D = torch.optim.lr_scheduler.LambdaLR(
        optim_D, lambda step: 1.0 - step / args.num_total_steps
    )
    ...
    for step in range(args.num_total_steps):
        ...
        metrics = torch_fidelity.calculate_metrics(
            input1=torch_fidelity.GenerativeModelModuleWrapper(
                G, args.z_size, args.z_type, num_classes
            ),
        ...
        tb.add_image(
            "observations", samples_vis, global_step=next_step, dataformats="HWC"
        )
        ...
        if metric_greater_cmp(metrics[leading_metric], last_best_metric):
            ...
            torch.jit.save(
                torch.jit.trace(G, (dummy_input,)),
                os.path.join(args.dir_logs, "generator.pth"),
            )
            torch.onnx.export(
                G,
                dummy_input,
