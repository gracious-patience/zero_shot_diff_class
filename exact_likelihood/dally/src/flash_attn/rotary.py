...
class RotaryEmbedding(torch.nn.Module):
    ...
    def _update_cos_sin_tables(self, x, seq_dimension=-2):
        ...
        if (
            ...
            self._cos_cached = repeat(
                torch.cos(freqs).to(x.dtype), "... d -> ... (d 2)"
            )
            self._sin_cached = repeat(
                torch.sin(freqs).to(x.dtype), "... d -> ... (d 2)"
            )

        return self._cos_cached, self._sin_cached

    def forward(
        self, q: torch.Tensor, k: torch.Tensor, seq_dimension=-2
...
class RotaryEmbedding2D(torch.nn.Module):
    ...
    def forward(self, q: torch.Tensor, k: torch.Tensor, seq_dimension=-2):
        ...
        q_emb, k_emb = torch.cat([q0_emb, q1_emb], dim=-1), torch.cat(
            [k0_emb, k1_emb], dim=-1
        )
        if seq_dimension == -3:
