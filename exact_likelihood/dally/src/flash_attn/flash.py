...
from dally.src.flash_attn.rotary import RotaryEmbedding, RotaryEmbedding2D
from dally.src.flash_attn.flash_attn_interface import flash_attn_func
from dally.src.flash_attn.bert_padding import unpad_input, pad_input, index_first_axis
...
class FlashAttention(nn.Module):
    ...
    def __init__(
        self, softmax_temp=None, attention_dropout=0.0, device=None, dtype=None
    ):
        super().__init__()
...
class FlashMHA(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        # dim_head added for compatibility
        assert batch_first
        ...
        assert self.embed_dim % heads == 0, "self.kdim must be divisible by num_heads"
        ...
        self.Wqkv = nn.Linear(dim, 3 * dim, bias=bias, **factory_kwargs)
