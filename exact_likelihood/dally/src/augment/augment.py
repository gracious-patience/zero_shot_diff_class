...
class AugmentPipe(torch.nn.Module):
    ...
    def forward(self, images, debug_percentile=None, use_cached=False):
        assert isinstance(images[0], torch.Tensor) and images[0].ndim == 4
        batch_size, num_channels, height, width = images[0].shape
        ...
        if not use_cached:
            ...
            if self.xflip > 0:
                i = torch.rand([batch_size], device=device) * 2
                params.append(i.clone().detach().view(-1, 1))
                i = torch.floor(i)
                i = torch.where(
            ...
            if self.rotate90 > 0:
                i = torch.rand([batch_size], device=device) * 4
                params.append(i.clone().detach().view(-1, 1))
                i = torch.floor(i)
                i = torch.where(
            ...
            if self.xint > 0:
                ...
                params.append(t.clone().detach())
                t = torch.where(t < self.xint * self.p, t, torch.zeros_like(t))
                if debug_percentile is not None:
            ...
            if self.scale > 0:
                ...
                params.append(s.clone().detach().view(-1, 1) - 1)
                s = torch.where(
        ...
        if torch.norm(G_inv - I_3) > 1e-5:
            ...
            images = [
                torch.nn.functional.pad(
                    input=x, pad=[mx0, mx1, my0, my1], mode="reflect"
                )
                for x in images
            ]
            ...
            # Upsample.
            images = [upfirdn2d.upsample2d(x=x, f=self.Hz_geom, up=2) for x in images]
            G_inv = (
            ...
            G_inv = (
                scale2d(2 / images[0].shape[3], 2 / images[0].shape[2], device=device)
                @ G_inv
            ...
            images = [grid_sample_gradfix.grid_sample(x, grid) for x in images]

            # Downsample and crop.
            images = [
                upfirdn2d.downsample2d(
                    x=x, f=self.Hz_geom, down=2, padding=-Hz_pad * 2, flip_filter=True
                )
                for x in images
            ]
        ...
        if not use_cached:
            ...
            if self.lumaflip > 0:
                ...
                i = torch.rand([batch_size, 1, 1], device=device) * 2
                params.append(i.clone().detach().view(-1, 1))
                i = torch.floor(i)
                i = torch.where(
            ...
            if self.hue > 0 and num_channels > 1:
                ...
                params.append(theta.clone().detach().view(-1, 1))
                theta = torch.where(
        ...
        # Execute if the transform is not identity.
        if torch.norm(C - I_4) > 1e-5:
            images = [
                x.reshape([batch_size, num_channels, height * width]) for x in images
            ]
            if num_channels == 3:
                images = [C[:, :3, :3] @ x + C[:, :3, 3:] for x in images]
            elif num_channels == 1:
                C = C[:, :3, :].mean(dim=1, keepdims=True)
                images = [
                    x * C[:, :, :3].sum(dim=2, keepdims=True) + C[:, :, 3:]
                    for x in images
                ]
            else:
        ...
        if self.imgfilter > 0:
            ...
            images = [
                x.reshape([1, batch_size * num_channels, height, width]) for x in images
            ]
            images = [
                torch.nn.functional.pad(input=x, pad=[p, p, p, p], mode="reflect")
                for x in images
            ]
            images = [
        ...
        if self.cutout > 0:
            size = (
                torch.rand([batch_size, 2, 1, 1, 1], device=device) * self.cutout_size
            )
            p = torch.rand([batch_size, 1, 1, 1, 1], device=device)
            size = torch.where(
                p < self.cutout * self.p,
                size,
                torch.zeros_like(size),
            )
            params.append(size.reshape(-1, 2).clone().detach())

            center = torch.rand([batch_size, 2, 1, 1, 1], device=device)

            params.append(center.reshape(-1, 2).clone().detach())
            if debug_percentile is not None:
            ...
            images = [x * mask for x in images]

        if not use_cached:
            self.params = torch.cat(params, dim=1)

        return images, self.params
