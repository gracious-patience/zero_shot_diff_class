...
class Trainer:
    ...
    def noise(self, img: Tensor, noise_level=0.1) -> Tensor:
        return self._ddpm.noise_scheduler.q_sample(img, torch.tensor(noise_level).cuda())[0]

    def denoise(self, noisy: Tensor, labels: Tensor, num_timesteps:int, noise_level=0.1, sampler_type='p_sample_loop') -> Tensor:
        sampler = Sampler(self.emas[-1].ema_model, noise_scheduler=self._ddpm.noise_scheduler, clamp=False)
        if sampler_type == 'p_sample_loop':
            return sampler.p_sample_loop((noisy.shape[0], 3, 32,32), labels=labels,noise=noisy, ode_steps=-1, start_t=noise_level, num_timesteps=num_timesteps)
        elif sampler_type == 'heun_solve':
            return sampler.heun_solve((noisy.shape[0], 3, 32,32), labels=labels,noise=noisy, ode_steps=-1, start_t=noise_level, num_timesteps=num_timesteps)