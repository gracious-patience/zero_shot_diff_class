...
class Trainer:
        ...
        if ddp.is_chief():
            if ckpt_path:
                ...
                if len(paths) > 0:
                    ...
                    logger.info("CHECKPOINT FOUND ON CHIEF. ckpt_path = {}".format(self.ckpt_path))
    ...
    def _sample(
        ...
        if self.config.trainer.conditional:
            labels = torch.randint(0, 11, size=(SIZE // chunk,), device=rank())
        else:
            labels = None
    ...
    def _train_step(self, batch):
        ...
        mask_probab = 0.1
        mask = torch.distributions.Bernoulli(probs=(1-mask_probab)).sample(labels.size())
        labels[~mask.bool()] = 10
